export const zhTW = {
  ...
  sources: {
    ...
    deleteSuccess: "來源刪除成功。注意：要從儲存中刪除檔案，必須在設定頁面中啟用“刪除檔案”选项。",
    ...
    batchUrlHint: '粘貼多個 URL（每行一個）進行批次導入',
  ...
  podcasts: {
    ...
    deleteProfileDesc: "這將移除 “{name}”。現有單集將保留其數據，但新單集將不再使用此設定。",
    outlineModel: "大綱模型",
  ...
  advanced: {
    ...
    rebuild: {
      ...
      whenToRebuildAns: "當您切換嵌入模型、升級模型版本、懷疑數據損壞或進行了大批次內容導入後，建議執行重建。",
      howLong: "重建需要多長時間？",
      howLongAns: "耗時取決於項目總數、模型速度和 API 速率限制。本地模型（如 Ollama）通常非常快。",
      isSafe: "在使用應用時重建安全嗎？",
      isSafeAns: "是的，重建過程是安全的。它不會刪除您的原始内容，僅會逐步替換向量數據。在大批次處理時，搜尋速度可能會有輕微抖動。",
    },
  },
  transformations: {
    title: "內容轉換規則",
