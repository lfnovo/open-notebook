...
- `build_source_context()` backs the source-chat graph: it requests the source's long context and adds insights as separate budgeted items. Full source text is retained when it fits. For an oversized source, up to 20% of the token budget is reserved for fitting insights in fetch order, unused space returns to the source, and the longest fitting source prefix carries an explicit truncation notice. If the budget cannot fit metadata, the notice, and at least one source character, the source item is omitted and metadata reports `source_text_status="omitted_budget"` rather than presenting notice-only text as source content.
