...
The Ask API provides programmatic access to the core 'ask' functionality of Open Notebook, allowing you to integrate its question-answering capabilities into other applications or workflows.
...
```json
{
  ...
  "strategy_model_id": "optional_strategy_model_id",
  "answer_model_id": "optional_answer_model_id",
  "final_answer_model_id": "optional_final_answer_model_id"
}
```

- `question` (str, required): The question you want to ask the notebook.
- `strategy_model_id` (str, optional): The ID of the model to use for the strategy generation phase. If not provided, the default strategy model configured in the system will be used.
- `answer_model_id` (str, optional): The ID of the model to use for generating intermediate answers. If not provided, the default answer model will be used.
- `final_answer_model_id` (str, optional): The ID of the model to use for generating the final consolidated answer. If not provided, the default final answer model will be used.

### Response Payload

The API will return a JSON response with the final answer:

```json
{
  "final_answer": "The generated answer to your question."
}
```

- `final_answer` (str): The final answer generated by the notebook after processing the question through the configured models and context.

### Prerequisites

1.  **Python Environment & Dependencies**:
    *   Ensure you have a Python environment (>=3.11, <3.13 recommended).
    *   Install the project dependencies. If you have cloned the repository, you can typically install it in editable mode along with its dependencies using:
        ```bash
        uv pip install -e .
        ```
    *   This will also install necessary API dependencies such as `fastapi` and `uvicorn`.

2.  **SurrealDB**:
    *   A SurrealDB instance must be running and accessible by the API server. You can start one using Docker:
        ```bash
        docker compose --profile db_only up
        ```

3.  **Environment Variables for SurrealDB**:
    *   The following environment variables must be set for the API to connect to SurrealDB:
        - `SURREAL_ADDRESS` (e.g., `localhost` or `127.0.0.1`)
        - `SURREAL_PORT` (e.g., `8000` or `8082` if using the `db_only` docker profile)
        - `SURREAL_USER` (e.g., `root`)
        - `SURREAL_PASS` (e.g., `root`)
        - `SURREAL_NAMESPACE` (e.g., `open_notebook`)
        - `SURREAL_DATABASE` (e.g., `main_db`)
    *   You can set these in a `.env` file in the project root.

4.  **AI Models Configuration**:
    *   AI models must be configured within the SurrealDB `model` table and the `open_notebook:default_models` record must be set up. This is typically handled by running the Streamlit application at least once or by manual setup in the database.
    *   Ensure the corresponding API keys for your chosen AI providers (e.g., `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, etc.) are set as environment variables.

### Running the API Server

You can run the API server using Uvicorn. From the project root directory:

```bash
uvicorn open_notebook.api.main:app --reload --port 8001
```
*(Note: The example uses port 8001 to avoid conflict if you are also running SurrealDB on the default port 8000, or the Streamlit app on its default port.)*
...
curl -X POST "http://localhost:8001/api/v1/ask" \
     ...
     -d '{
           ...
           "strategy_model_id": "openai:gpt-4o",
           "final_answer_model_id": "anthropic:claude-3-opus-20240229"
         }'
```
...
```json
{
  "final_answer": "The main challenges in quantum computing include qubit decoherence, error correction, scalability of quantum processors, and the development of quantum algorithms..."
}
```
