...
export function NoteEditorDialog({ open, onOpenChange, moduleId, note }: NoteEditorDialogProps) {
  ...
  const onSubmit = async (data: CreateNoteFormData) => {
    if (note) {
      ...
      // Only invalidate module-specific queries if we have a moduleId
      if (moduleId) {
        queryClient.invalidateQueries({ queryKey: QUERY_KEYS.notes(moduleId) })
      }
    } else {
      // Creating a note requires a moduleId
      if (!moduleId) {
        console.error('Cannot create note without module_id')
        return
      }
