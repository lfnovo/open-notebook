export const jaJP = {
  ...
  connectionErrors: {
    ...
    apiDesc: "SAI Notebook APIサーバーに到達できませんでした",
  ...
  sources: {
    ...
    includeLinkedPages: "リンク先ページを含める",
    includeLinkedPagesHint: "このページのリンクをスキャンし、個別のソースとして取り込むリンクを選択します",
    selectLinks: "リンクを選択",
    selectLinksDescription: "個別のソースとして取り込むリンク先ページを選択してください",
    scanningLinks: "ページのリンクをスキャンしています…",
    linksFound: "{count} 件のリンクが見つかりました",
    noLinksFound: "このページにリンクは見つかりませんでした。元のページのみが取り込まれます。",
    selectAll: "すべて選択",
    selectNone: "選択を解除",
    selectSameDomain: "同じサイトを選択",
    sameDomainBadge: "同じサイト",
    externalBadge: "外部",
    discoverLinksError: "ページのリンクをスキャンできませんでした。",
    skipImportOriginal: "スキップ — 元のページのみ取り込む",
    addSource: "ソースを追加",
  ...
  settings: {
    ...
    filesHelp: "ファイルがアップロードされて処理されると、ファイル自体は不要になります。ほとんどのユーザーはSAI Notebookがアップロードフォルダから自動的にファイルを削除することを許可すべきです。",
  ...
  advanced: {
    ...
    updateAvailableDesc: "SAI Notebookの新しいバージョンが利用可能です。",
    upToDate: "最新版です",
    unknown: "不明",
  ...
  models: {
    ...
    defaultAssignmentsDesc: "SAI Notebook全体で異なる目的に使用するモデルを設定",
    missingRequiredModels: "必須モデルがありません: {models}。これらがないとSAI Notebookが正しく機能しない可能性があります。",
  ...
  apiKeys: {
    ...
    description: "APIキーをデータベースに安全に保存し、SAI NotebookでAIプロバイダーを有効にします。",
