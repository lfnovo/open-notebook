export const jaJP = {
  ...
  connectionErrors: {
    ...
    apiDesc: "AgentBook APIサーバーに到達できませんでした",
  ...
  settings: {
    ...
    filesHelp: "ファイルがアップロードされて処理されると、ファイル自体は不要になります。ほとんどのユーザーはAgentBookがアップロードフォルダから自動的にファイルを削除することを許可すべきです。",
  ...
  advanced: {
    ...
    updateAvailableDesc: "AgentBookの新しいバージョンが利用可能です。",
    upToDate: "最新版です",
    unknown: "不明",
  ...
  models: {
    ...
    defaultAssignmentsDesc: "AgentBook全体で異なる目的に使用するモデルを設定",
    missingRequiredModels: "必須モデルがありません: {models}。これらがないとAgentBookが正しく機能しない可能性があります。",
  ...
  apiKeys: {
    ...
    description: "APIキーをデータベースに安全に保存し、AgentBookでAIプロバイダーを有効にします。",
