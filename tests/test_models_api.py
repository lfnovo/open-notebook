...
class TestUpdateDefaultModels:
    ...
    def _mock_defaults(self):
        ...
        defaults.default_image_generation_model = "model:image"
    ...
    def test_absent_field_keeps_current_value(self, client):
        ...
        assert defaults.default_image_generation_model == "model:image"
        assert response.json()["default_transformation_model"] == "model:transform"

    def test_explicit_null_clears_image_generation_default(self, client):
        defaults = self._mock_defaults()
        response = self._put(
            client, defaults, {"default_image_generation_model": None}
        )

        assert response.status_code == 200
        assert defaults.default_image_generation_model is None
        defaults.update.assert_awaited_once()
        assert response.json()["default_image_generation_model"] is None
