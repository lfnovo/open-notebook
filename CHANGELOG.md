...
### Fixed
- Podcast generation now rejects incompatible voices for the supported Google Gemini TTS preview models before persisting an episode or generating its outline and transcript. The error identifies the speaker, voice, and effective model, including per-speaker model overrides; other providers and unknown model catalogues remain permissive (#1238)
