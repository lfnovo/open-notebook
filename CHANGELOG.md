...
### Fixed
- Sources list can now be sorted by every column — type, title, created, updated, insights count, and embedded — each header toggles asc/desc, and an "Updated" column was added. The API validates `sort_by` against a whitelist and projects a computed `type` field so SurrealDB can order by it (#895, #930)
