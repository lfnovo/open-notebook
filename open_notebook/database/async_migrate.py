...
def _migration_number(path: Path) -> int:
    return int(path.name.split("_", 1)[0].split(".", 1)[0])


def _load_numbered_migrations(*, down: bool) -> list["AsyncMigration"]:
    suffix = "_down.surrealql" if down else ".surrealql"
    paths = [
        path
        for path in MIGRATIONS_DIR.glob(f"*{suffix}")
        if path.name.split("_", 1)[0].split(".", 1)[0].isdigit()
        and (down or not path.name.endswith("_down.surrealql"))
    ]
    return [
        AsyncMigration.from_file(str(path))
        for path in sorted(paths, key=_migration_number)
    ]
...
class AsyncMigrationManager:
    ...
    def __init__(self):
        ...
        self.up_migrations = _load_numbered_migrations(down=False)
        self.down_migrations = _load_numbered_migrations(down=True)
