...
function extractSection(content: string, heading: string): string {
  const re = new RegExp(`#{1,4}\\s*\\*?\\*?${heading}[^\\n]*\\n([\\s\\S]*?)(?=\\n#{1,4}\\s|\\n---\\n|$)`, 'i')
...
// Parse "Use Cases" or "Potential Use Cases" bullets
function parseUseCases(content: string): { title: string; desc: string }[] {
  const section = extractSection(content, 'Use Cases') || extractSection(content, 'Potential Use Cases')
  if (!section) return []
  return extractBullets(section)
    .filter(b => b.label || b.detail)
    .map(b => ({ title: b.label || b.detail.split('.')[0], desc: b.detail }))
}
...
// Parse "Data Structure" / field descriptions
function parseFields(content: string): { name: string; example: string }[] {
  ...
  return extractBullets(section)
    .filter(b => b.label)
    .map(b => {
      const exMatch = b.detail.match(/e\.g\.,?\s*`?([^`)]+)`?/)
      return { name: b.label, example: exMatch ? exMatch[1].trim() : b.detail.slice(0, 60) }
    })
}

// Parse example request/response block
function parseExample(content: string): { request: string; response: string } | null {
  const reqMatch = content.match(/\*\*Request\*\*[:\s]*\n([^\n]+)/)
  const resMatch = content.match(/```[a-z]*\n([\s\S]*?)```/)
  if (!reqMatch && !resMatch) return null
  return {
    request:  reqMatch ? reqMatch[1].replace(/"/g, '').trim() : '',
    response: resMatch ? resMatch[1].trim() : '',
  }
}
...
// Parse "Notable Observations" section
function parseObservations(content: string): { title: string; detail: string }[] {
  ...
  return extractBullets(section)
    .filter(b => b.label || b.detail)
    .map(b => ({ title: b.label || b.detail.split('.')[0], detail: b.detail }))
}

// Parse "Missing/Placeholder Data" section
...
// ── SECTION-based parsers (SECTION X — HEADING format) ───────────────────────

function extractSectionByNumber(content: string, headingPattern: string): string {
  const re = new RegExp(
    `SECTION\\s+\\d+\\s*[—–-]+\\s*${headingPattern}[^\\n]*\\n[-\\s]*\\n?([\\s\\S]*?)(?=\\n[-]{3,}\\nSECTION|\\n[-]{10,}|$)`,
    'i'
  )
  const m = content.match(re)
  return m ? m[1].trim() : ''
...
interface EntityRow { entity: string; period: string; count: string; amount: string }
function parseEntityTable(content: string): EntityRow[] {
  const section = extractSectionByNumber(content, 'ENTITY[\\s\\S]{0,20}ACTIVITY')
  if (!section) return []
  ...
  for (const line of section.split('\n')) {
    ...
    if (/^[-:]+$/.test(cells[0]) || /entity/i.test(cells[0])) continue
    rows.push({ entity: cells[0], period: cells[1], count: cells[2], amount: cells[3] })
  }
  return rows
}
...
function parseChartData(content: string): ChartSeries[] {
  const section = extractSectionByNumber(content, 'CHART[\\s\\S]{0,20}DATA')
  if (!section) return []
  ...
  for (const line of section.split('\n')) {
    ...
    if (headerMatch) {
      ...
      current = { label: headerMatch[1].trim(), data: [] }
      continue
    }
    const dataMatch = trimmed.match(/^-\s*(\d{4}-\d{2}):\s*[₹]?([\d,]+\.?\d*)/)
    if (dataMatch && current) {
      current.data.push({ period: dataMatch[1], value: parseFloat(dataMatch[2].replace(/,/g, '')) || 0 })
    }
  }
  if (current) series.push(current)
  return series
...
function parseFinalInsights(content: string): string[] {
  const section = extractSectionByNumber(content, 'FINAL[\\s\\S]{0,20}INSIGHT')
  if (!section) return []
...
function parseBehavioralShifts(content: string): string {
  const section = extractSectionByNumber(content, 'BEHAVIORAL[\\s\\S]{0,20}SHIFT')
  return section.replace(/\*\*/g, '').replace(/\n+/g, ' ').trim()
}

function parseAnomalyTimeline(content: string): string {
  const section = extractSectionByNumber(content, 'ANOMALY[\\s\\S]{0,20}TIMELINE')
  return section.replace(/\*\*/g, '').replace(/\n+/g, ' ').trim()
}

// ── UI primitives ─────────────────────────────────────────────────────────────

function ServiceTypeCard({ type, label, color, icon: Icon }: {
  type: string; label: string; color: string; icon: React.ElementType
}) {
  return (
  ...
  return (
    <div className="flex items-start gap-3 py-2.5 border-b last:border-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        {example && (
          <span className="ml-2 text-[10px] font-mono bg-muted px-1.5 py-0.5 rounded text-muted-foreground">
            e.g. {example}
          </span>
        )}
      </div>
...
export function TimelineAnalysisInsightViewer({ content }: TimelineAnalysisInsightViewerProps) {
  const data = useMemo(() => {
    ...
    // SECTION-based structured report parsers
    ...
    return {
      ...
      entityRows, entityInsights, chartSeries, finalInsights,
      behavioralShift, anomalyTimeline,
    }
  }, [content])
    ...
    <div className="space-y-5 pb-4">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-slate-900 via-indigo-950 to-violet-950 text-white p-6 shadow-lg">
        ...
        <div className="relative">
          ...
          <h2 className="text-2xl font-black text-white mb-1">Communication Log Report</h2>
          <p className="text-sm text-violet-200/70 mb-5">Telecom / Network Event Log — Structured Breakdown &amp; Analysis</p>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {data.destTokens.slice(0, 3).map((t, i) => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {data.geoExamples.length > 0 && (
            <SectionCard icon={MapPin} title="Geolocation Samples" iconClass="text-rose-500">
              ...
              <p className="text-[10px] text-muted-foreground mt-2">Format: latitude/longitude</p>
            </SectionCard>
          )}
          {data.tsExamples.length > 0 && (
            <SectionCard icon={Clock} title="Timestamp Samples" iconClass="text-indigo-500">
              ...
              <p className="text-[10px] text-muted-foreground mt-2">Format: YYYY-MM-DD HH:MM:SS</p>
            </SectionCard>
          )}
        </div>
      )}
      ...
      {data.nextSteps.length > 0 && (
        <SectionCard icon={Users} title="Next Steps to Help You" iconClass="text-emerald-500">
          <div className="space-y-2">
            {data.nextSteps.map((step, i) => (
              <div key={i} className="flex items-start gap-3 p-2.5 rounded-lg bg-emerald-50/50 dark:bg-emerald-950/10 border border-emerald-100 dark:border-emerald-900/30">
                <span className="w-5 h-5 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400 flex items-center justify-center text-[10px] font-black shrink-0 mt-0.5">
                  {i + 1}
                </span>
      ...
      {data.example && (
        <SectionCard icon={FileCode} title="Example Request &amp; Response" iconClass="text-blue-500">
          ...
          {data.example.response && (
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5">Response (CSV)</p>
              <pre className="text-[10px] font-mono bg-slate-900 text-emerald-400 p-3 rounded-lg overflow-x-auto leading-relaxed">
                {data.example.response}
              </pre>
            </div>
          )}
              ...
              <tbody>
                {data.entityRows.map((row, i) => (
                  <tr key={i} className="border-b last:border-0 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-2.5 tabular-nums text-center">{row.count}</td>
                    <td className="px-4 py-2.5 tabular-nums font-bold text-emerald-600 dark:text-emerald-400">{row.amount}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {/* ── Anomaly Timeline + Behavioral Shifts ── */}
      {(data.anomalyTimeline || data.behavioralShift) && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {data.behavioralShift && (
            <SectionCard icon={Activity} title="Behavioral Shifts" iconClass="text-violet-500">
              <p className="text-xs text-muted-foreground leading-relaxed">{data.behavioralShift}</p>
            </SectionCard>
          )}
        </div>
      )}

      {/* ── Chart Data (SECTION 11) ── */}
      {data.chartSeries.length > 0 && (
        <SectionCard icon={Activity} title="Chart Data — Period Breakdown" iconClass="text-indigo-500">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {data.chartSeries.map((series, si) => {
              ...
              const isCount = /volume|anomal/i.test(series.label)
              return (
                <div key={si}>
                        ...
                        <div key={di}>
                          <div className="flex justify-between text-xs mb-1">
                            ...
                            <span className="text-muted-foreground tabular-nums">
                              {isCount ? d.value : `₹${d.value.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`}
                            </span>
                          </div>
      ...
      {/* ── Final Insights (SECTION 12) ── */}
      {data.finalInsights.length > 0 && (
        <SectionCard icon={Target} title="Final Insights" iconClass="text-emerald-500">
          <div className="space-y-2">
...
// NOTE: SECTION-based render sections are injected via the main viewer above.
// The sections below are rendered conditionally inside BankAnalysisInsightViewer pattern.
