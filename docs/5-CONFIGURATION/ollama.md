...
Ollama provides free, local AI models that run on your own hardware. This guide covers everything you need to know about setting up Ollama with SAI Notebook, including different deployment scenarios and network configurations.
...
When both SAI Notebook and Ollama run directly on your machine:
...
### Scenario 2: SAI Notebook in Docker, Ollama on Host

When SAI Notebook runs in Docker but Ollama runs on your host machine:
...
When both SAI Notebook and Ollama run in the same Docker Compose stack:
...
This is the most common cause of "Failed to send message" errors. SAI Notebook requires the **exact model name** as it appears in Ollama.
...
**Step 2: Use the exact name when adding the model in SAI Notebook**
...
**Note:** Some models use `:latest` as the default tag. If `ollama list` shows `model:latest`, you must use `model:latest` in SAI Notebook, not just `model`.

**Step 3: Configure in SAI Notebook**
...
**1. "Ollama unavailable" in SAI Notebook**
...
```bash
# If SAI Notebook runs in Docker or on a different machine,
# Ollama must bind to all interfaces, not just localhost
export OLLAMA_HOST=0.0.0.0:11434
ollama serve
```
> **Why this is needed:** By default, Ollama only accepts connections from `localhost` (127.0.0.1). When SAI Notebook runs in Docker or on a different machine, it can't reach Ollama unless you configure `OLLAMA_HOST=0.0.0.0:11434` to accept external connections.
...
**From inside SAI Notebook container, test Ollama:**
```bash
# Get into container
...
1. **Model name mismatch**: The model name in SAI Notebook doesn't exactly match `ollama list`
2. **No default model configured**: You haven't set a default chat model in Settings → Models
3. **Model was deleted**: You removed the model from Ollama but didn't update SAI Notebook's defaults
4. **Model record deleted**: The model was removed from SAI Notebook but is still set as default
...
# Compare with what's configured in SAI Notebook
...
1. Refresh the SAI Notebook page
...
# SAI Notebook configuration (configure via Settings → API Keys instead)
...
This comprehensive guide should help you successfully deploy and optimize Ollama with SAI Notebook. Start with the Quick Start section and refer to specific scenarios as needed.