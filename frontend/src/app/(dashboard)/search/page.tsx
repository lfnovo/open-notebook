  ...
  return (
    <AppShell>
      <div className="p-4 md:p-6">
        <h1 className="text-xl md:text-2xl font-bold mb-4 md:mb-6">{t.search.askAndSearch}</h1>
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'ask' | 'search')} className="w-full space-y-6">
          ...
          <TabsContent value="ask" className="mt-6">
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">{t.search.askYourKb}</CardTitle>
                <p className="text-sm text-muted-foreground">
                  {t.search.askYourKbDesc}
                </p>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    id="ask-question"
                    placeholder={t.search.enterQuestionPlaceholder}
                    value={askQuestion}
          ...
          <TabsContent value="search" className="mt-6">
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">{t.search.search}</CardTitle>
                <p className="text-sm text-muted-foreground">
                  {t.search.searchDesc}
                </p>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">{t.search.pressToSearch}</p>
                </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {!hasEmbeddingModel && (
                      <div className="flex items-center gap-2 text-sm text-amber-600 dark:text-amber-500">
                        <AlertCircle className="h-4 w-4" />
                        <span>{t.search.vectorSearchWarning}</span>
                      </div>
                    )}
                    <RadioGroup
                      value={searchType}
                  ...
                  <div className="mt-6 space-y-3">
                    <div className="flex items-center justify-between">
                      <h3 className="text-sm font-medium">
                        {t.search.resultsFound.replace('{count}', searchMutation.data.total_count.toString())}
                      </h3>
                      <Badge variant="outline">{searchMutation.data.search_type === 'text' ? t.search.textSearch : t.search.vectorSearch}</Badge>
                    </div>
                              ...
                              {result.matches && result.matches.length > 0 && (
                                <Collapsible className="mt-3">
                                  <CollapsibleTrigger className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
                                    ...
                                    {t.search.matches.replace('{count}', result.matches.length.toString())}
                                  </CollapsibleTrigger>
