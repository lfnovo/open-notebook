    ...
    for i, speaker in enumerate(st.session_state.dialog_speakers):
        with st.container(border=True):
            ...
            with col_delete:
                ...
                if st.button("🗑️", key=f"delete_speaker_{i}", help=f"Delete Speaker {i+1}", disabled=len(st.session_state.dialog_speakers) <= 1):
            ...
            with col_spk1:
                speaker["name"] = st.text_input("Name*", value=speaker.get("name", ""), key=f"dialog_speaker_{i}_name")
                speaker["voice_id"] = st.text_input("Voice ID*", value=speaker.get("voice_id", ""), key=f"dialog_speaker_{i}_voice")
            
            with col_spk2:
                speaker["backstory"] = st.text_area("Backstory*", value=speaker.get("backstory", ""), key=f"dialog_speaker_{i}_backstory")
                speaker["personality"] = st.text_area("Personality*", value=speaker.get("personality", ""), key=f"dialog_speaker_{i}_personality")
            ...
            if st.button("✅ Assign Speaker", type="primary"):
                if episode_context and selected_speaker:
                    ...
                    if current_episode:
                        ...
                        success = asyncio.run(update_episode_profile(episode_context, updated_data))
                        if success:
                ...
                if sp_name and valid_speakers:
                    ...
                    if mode == "create":
                        ...
                        if success:
                            ...
                            if pending_episode:
                                episode_profiles = asyncio.run(fetch_episode_profiles())
                                current_episode = next((ep for ep in episode_profiles if ep['id'] == pending_episode), None)
                                if current_episode:
                                    ...
                                    assign_success = asyncio.run(update_episode_profile(pending_episode, updated_data))
                                    if assign_success:
